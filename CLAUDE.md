...
- **`sources/` is read-only inside a sandboxed session.** It is `--ro-bind`ed on top of the writable cwd, because the shell sources it on every launch. Editing it needs `CLAUDE_SANDBOX_UNLOCK_CONFIG=1 __claude` (the `cu` alias) or a plain terminal — a write otherwise fails with `EROFS`. The same applies to `$PWD/.direnv`, `$PWD/.git/{hooks,config}`, `lefthook*.yml`, and direnv's `source_url` CAS: all host-executed, all re-pinned read-only.
