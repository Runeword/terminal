...
- `packages.firefox-mcp` — a standalone MCP server package from `packages/custom/`, kept as a Nix derivation because it also needs a sidecar binary on PATH (geckodriver). The other three MCP servers are **not** Nix-packaged: `nix-mcp`, `aws-api-mcp`, and `google-workspace-mcp` are launched via `uvx` (pinned version, fetched from PyPI at run time) straight from their plugin `.mcp.json`, exactly as `figma-mcp` is launched via `npx`; the `uv`/`nodejs`/`python312` runtimes they need sit on claude's PATH (see `wrappers/claude.nix`). The other `packages/custom/` entries are Go binaries with no flake output of their own. `packages/custom/default.nix` puts these into the tools env (so they're on PATH inside the terminal): `git-branches`, `claude-session-status`, `claude-context`, `fm-query` (the fzf-query compiler shared by `sources/`'s `fm_rg.sh` and `fm_preview.sh`), `git-hunk-pick` and `git-stash-hunks` (split a diff into selectable hunks, and turn a picked subset into one stash — back the `grd`/`gru`/`gsp` hunk pickers in `shell/functions/git.bash`), `claude-seccomp-bpf`, `claude-ssh-sanitize`, and `claude-cwd-gate` (the fail-closed helpers the sandbox launcher invokes by bare name — see the claude sandbox section), and `leader-aliases` (renders `sources/.config/shell/leader.toml` into the fzf rows behind the leader-key picker in `shell/functions/aliases.sh`). `claude-statusline`, `claude-docs-guard`, `git-allowlist-hook`, and `git-shim` are consumed only inside the `claude` wrapper (see below).
...
On Linux, `__claude` (in `sources/.config/shell/functions/claude.bash`) never runs `claude` directly — it prefixes the launcher above, which re-execs into an unprivileged bubblewrap namespace. **The gate fails closed**: a missing `bwrap` or a non-executable launcher aborts with a message rather than starting an unsandboxed claude, and `CLAUDE_SANDBOX=0` is the explicit escape hatch, not a fallback. Claude Code's own `/sandbox` is not used here because its Linux seccomp filter blocks `socket(AF_UNIX, …)`, which severs the nix-daemon socket; it also only ever wrapped Bash, while this wraps the whole process tree — file tools, hooks, and MCP servers alike. Three security-critical steps are extracted from the launcher into fail-closed, table-tested Go binaries on the tools-env PATH that it invokes by bare name — `claude-cwd-gate` (the cwd refusal gate), `claude-ssh-sanitize` (builds the sanitized `~/.ssh` copy), and `claude-seccomp-bpf` (emits the TIOCSTI/TIOCLINUX seccomp filter) — each aborting the launch if missing rather than starting a weaker sandbox; the rest of the launcher is bwrap orchestration and stays bash.
...
- **A Go binary (`packages/custom/<name>/`)** — add `main_test.go` beside `main.go`. `pkgs.buildGoModule` runs `go test` in its check phase, so the test runs on every build with no extra flake wiring — the binary is a build dependency of the wrappers/terminal that the checks build. Follow the table-driven, stdlib-`testing` style that every binary in `packages/custom/` now ships: `git-shim`, `git-allowlist-hook`, `claude-docs-guard`, `claude-session-status`, `claude-context`, `fm-query`, `git-hunk-pick`, `git-stash-hunks`, `claude-seccomp-bpf` (whose BPF filter a deterministic decode test verifies), `claude-ssh-sanitize`, `claude-cwd-gate`, `claude-statusline`, `git-branches`, and `leader-aliases`.
