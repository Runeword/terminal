...
__load_compinit() {
  if (( __compinit_loaded == 0 )); then
    ...
    # The dump only goes stale when the set of completion functions changes,
    # which on Nix means the resolved (store-path) fpath changes. While the
    # fpath it was built from is unchanged, load it unchecked (-C) from its
    # byte-compiled .zwc; otherwise rebuild, recompile and restamp it. (An age
    # check can't do this: compinit leaves an unchanged dump's mtime alone, so
    # "older than 24h" stays true and every shell took the slow path.)
    local -a __fp=( ${fpath:A} )
    local key="$ZSH_VERSION ${__fp[*]}" stamp="$ZCOMPDUMP.fpath" line=
    [[ -r $stamp ]] && read -r line < "$stamp"
    if [[ -s $ZCOMPDUMP && -s $ZCOMPDUMP.zwc && $line == "$key" ]]; then
      compinit -C -d "$ZCOMPDUMP"
    else
      command rm -f -- "$ZCOMPDUMP" "$ZCOMPDUMP.zwc" 2>/dev/null
      compinit -d "$ZCOMPDUMP"
      { zcompile "$ZCOMPDUMP" && print -r -- "$key" >| "$stamp"; } 2>/dev/null
    fi
...
if command -v dircolors >/dev/null 2>&1; then
  typeset -g DIRCOLORS_BIN="${commands[dircolors]}"
...
# Wrap the zle widgets once, on the first precmd, instead of re-wrapping every
# widget before every prompt (~10-15ms each time). All widgets exist by then:
# this file defines them (navi/zoxide/fzf included, see Plugins) and the
# compinit precmd hook runs before autosuggestions' own.
...
# Tool init scripts are cached rather than generated by spawning the tool on
# every start. Usage: __init_cache <tool> <generator...>; sets REPLY to the
# cache file. Its first line records the generator and the tool's store path,
# so an upgrade (new store path) or a changed generator regenerates it. Only a
# successful generation is cached; if the cache dir is read-only (the claude
# sandbox), an existing, possibly stale cache is still used.
__init_cache() {
  local bin="${commands[$1]}" key line out
  [[ -n $bin ]] || return 1
  REPLY="${XDG_CACHE_HOME:-$HOME/.cache}/zsh/$1.zsh" key="# $bin ${*:2}"
  [[ -r $REPLY ]] && read -r line < "$REPLY"
  [[ $line == "$key" ]] && return
  out=$("${@:2}") || return 1
  { print -r -- "$key"$'\n'"$out" >| "$REPLY"; } 2>/dev/null || [[ -r $REPLY ]]
}

# starship's init sets PROMPT2="$(starship prompt --continuation)", i.e. one
# more starship spawn on every start; single-quote it so it renders lazily,
# only when a continuation line is actually shown (promptsubst is on).
__starship_init() { starship init zsh | sed "s/^PROMPT2=\"\(.*\)\"\$/PROMPT2='\1'/"; }

# Load critical plugins immediately (needed for prompt/env)
typeset -F __T5="$SECONDS"
__init_cache starship __starship_init && source "$REPLY"
typeset -F __T6="$SECONDS"
_profile "starship: %.0fms\n" $(( (__T6 - __T5) * 1000 ))

typeset -F __T7="$SECONDS"
__init_cache direnv direnv hook zsh && source "$REPLY"
...
# Non-critical plugins, from cached init scripts. Loaded here, not on the
# first precmd: precmd hooks run before the first prompt is drawn anyway, and
# their widgets must exist when autosuggestions wraps widgets (only once, on
# the first precmd, with ZSH_AUTOSUGGEST_MANUAL_REBIND).
typeset -F __TD1="$SECONDS"
__init_cache navi navi widget zsh && source "$REPLY"
typeset -F __TD2="$SECONDS"
__init_cache zoxide zoxide init --no-cmd zsh && source "$REPLY"
typeset -F __TD3="$SECONDS"
# fzf's own share dir, resolved from its binary: no `fzf-share` spawn.
if [[ ${options[zle]} = on && -n ${commands[fzf]} ]]; then
  __fzf_bin="${commands[fzf]:A}"
  source "${__fzf_bin%/bin/fzf}/share/fzf/key-bindings.zsh"
  unset __fzf_bin
fi
typeset -F __TD4="$SECONDS"
_profile "navi: %.0fms, zoxide: %.0fms, fzf: %.0fms\n" \
  $(( (__TD2 - __TD1) * 1000 )) \
  $(( (__TD3 - __TD2) * 1000 )) \
  $(( (__TD4 - __TD3) * 1000 ))
