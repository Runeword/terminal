...
- **`sources/` is read-write inside a sandboxed session, and that is a deliberate hole.** `$PERMEANCE_TREE` is bound read-write, so an edit there is live on the host at once: the next host shell sources it, tmux runs it, and `claude.bash` executes the launcher from it, before anyone has reviewed it. `sources/` is git-tracked, so review is `git diff -- sources` and undo is `git checkout -- sources`, both on the host. One exception: `.claude/git-allowlist.toml` is re-pinned read-only from the host copy (the git-shim re-reads it on every call, so a writable copy would let a session widen its own allowlist), and it is edited from a plain terminal. In bundled mode `$PERMEANCE_TREE` is a store path and nothing is bound. What stays re-pinned read-only unconditionally, with no flag to lift it: `$PWD/.direnv`, `$PWD/.git/{hooks,config}`, `lefthook*.yml`, and direnv's `source_url` CAS — host-executed too, and never what a config-editing session needs. (Cost: `git config`/`git remote` writes are refused inside the sandbox; reads, staging, and checkout are unaffected.) An overlay with a host-side apply step (`claude-sources-apply`) guarded the tree until 2026-09-18 and was removed as too much workflow; it is in git history if the trade-off ever changes.
...
Edit it like any other directory: Edit/Write, `shellcheck`, `bash -n`, `nix build .#checks.x86_64-linux.<wrapper>` and `smoke` all see the edits, and so does the host, immediately (a *new* file must be `git add`-ed before nix sees it, as for any flake). A syntax error in `.zshrc`, `claude.bash` or the launcher breaks the next host shell, so run `bash -n` and `shellcheck` on a shell file right after editing it. `git diff` / `git status` are blocked by the git-shim inside the sandbox, so at handover say what changed and that `git diff -- sources` on the host shows it.
...
Only `.claude/git-allowlist.toml` needs a plain terminal. The rest of the repo — `CLAUDE.md`, `flake.nix`, `wrappers/`, `packages/`, `lib/`, `devshells/`, `modules/`, `overlays/`, `infra/` — is directly writable. `.git/config`, `.git/hooks`, and `lefthook*.yml` are refused unconditionally and no workflow lifts them.
