...
- `packages.firefox-mcp` / `packages.mobile-mcp` / `packages.aws-api-mcp` / `packages.google-workspace-mcp` — standalone MCP server packages from `packages/custom/`. Other `packages/custom/` entries (`claude-statusline`, `claude-session-status`, `claude-docs-guard`, `git-allowlist-hook`, `git-shim`) are Go binaries built and consumed internally by the `claude` wrapper / `sources/.claude/settings.json` hooks — not exposed as flake outputs.
...
Two overlays are applied: one pins `awscli2` to nixpkgs-24.05; one source-pins `tmux` to 3.6a via `overrideAttrs` + `fetchFromGitHub` (so we don't carry a second nixpkgs input just to freeze the version). Both follow the `final: prev:` convention — `prev` is used when redefining existing attributes.
...
OpenTofu config managing GitHub repository settings (visibility, Actions permissions, branch protection, security analysis) via the `integrations/github` provider. State is local (`*.tfstate` gitignored) — bootstrap is import-based, not greenfield apply, since the repository already exists. The `infra` devshell helper runs `tofu` against this directory; see `infra/README.md` for the import sequence.
